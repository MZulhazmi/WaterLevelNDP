...
void SendDataSim800LTask(void *pvParameters) {
  ...
  if (sendData()) {
    ...
    if (now >= 1700000000) {
      ...
      // Hitung kapan kirim berikutnya (kelipatan 15 menit berikutnya)
      int currentMinute = timeinfo.tm_min;
      int nextSendMinute = ((currentMinute / 15) + 1) * 15;
      
      // Jika sudah melewati 45, next adalah 00 jam berikutnya
      if (nextSendMinute >= 60) {
        nextSendMinute = 0;
        timeinfo.tm_hour++;
      ...
      timeinfo.tm_min = nextSendMinute;
      timeinfo.tm_sec = 0;
      
      time_t nextSendTime = mktime(&timeinfo);
